...
cdef extern from "simple_solver.h":
    ...
    double dengo_evolve_simple (double dtf, double &dt, double z,
...
def run_simple(ics, double tf, int niter = 10000,
    ...
    #ttot = dengo_evolve_simple(tf, dt, input, rtol, atol, dims, data)
    ...
    field_data.dengo_data_file = "simple_tables.h5"

    field_data.reltol = reltol;
    field_data.floor_value = floor_value;

    for iter in range(niter):
        status = simple_solve_chemistry( units, field_data, dt);
        j = 0;

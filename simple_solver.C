...
simple_data *simple_setup_data( const char *FileLocation, int *NumberOfFields, char ***FieldNames)
{
    ...
    //-----------------------------------------------------
    // Function : simple_setup_data
    // Description: Initialize a data object that stores the reaction/ cooling rate data 
    //-----------------------------------------------------

    int i, n;
    
    simple_data *data = (simple_data *) malloc(sizeof(simple_data));
    
    // point the module to look for simple_tables.h5
...
void simple_read_rate_tables(simple_data *data)
{
    const char * filedir;
    if (data->dengo_data_file != NULL){
        filedir =  data->dengo_data_file; 
    } else{
        filedir = "simple_tables.h5";   
    }
...
void simple_read_cooling_tables(simple_data *data)
{
...
void simple_read_gamma(simple_data *data)
{
...
void simple_interpolate_rates(simple_data *data,
                    int nstrip)
{
...
void simple_interpolate_gamma(simple_data *data,
                    int i)
{   
...
int simple_main(int argc, char** argv )
{
    ...
    //              and write the result to simple_solution.h5 if output file name is not specified
    ...
    //-----------------------------------------------------
    simple_data *data = simple_setup_data(NULL, NULL, NULL);

    /* Initial conditions */
    hid_t file_id;
    if (argc < 2){
    file_id = H5Fopen("simple_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "simple_initial_conditions.h5 so dying.\n");
        return(1);}
    } else {
    ...
    double ttot;
    int flag = dengo_evolve_simple(dtf, dt, z, input, rtol, atol, dims, data, temp);
    if (flag > 0){
    ...
    if (argc < 3){
        file_id = H5Fcreate("simple_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    } else{
...
int dengo_evolve_simple (double dtf, double &dt, double z, double *input,
            double *rtol, double *atol, unsigned long dims, simple_data *data, double *temp_array ){

    //-----------------------------------------------------
    // Function     : dengo_evolve_simple
    ...
    //                  data    : simple_data object that relay the reaction/cooling rates, and normalizations 
    ...
    rhs_f f = calculate_rhs_simple;

    #ifndef CVSPILS
    #ifdef  CVKLU
    jac_f jf = calculate_sparse_jacobian_simple;
    #else
    jac_f jf = calculate_jacobian_simple;
    #endif
    #endif
    
    #ifdef CVSPILS
    jac_f jf = calculate_JacTimesVec_simple;
    #endif
    ...
    int d;
    int sum;
    int NSPARSE = 9; // no. of sparse jacobian compoents
    ...
    for (int d = 0; d < ntimes; d++){
        ...
        simple_calculate_temperature(data, &input[d*v_size], nstrip, N);
    ...
    if ( v_size_res > 0 ){
        ...
        simple_calculate_temperature(data, &input[d*v_size], nstrip_res, N);
...
double evolve_in_batches( void * cvode_mem, N_Vector y_vec, N_Vector abstol,  
    ...
    //                  data            : simple_data object that stores rate/ temperature/ scale data
...
int simple_calculate_temperature(simple_data *data,
                        double *input, int nstrip, int nchem)
{
    ...
    for ( i = 0; i < nstrip; i++ ){
        ...
        */
        ...
        // TODO: pull the rates from simple_data
	...
	// assume these species has negligible abundance....
        ...
        density = 2.0*H2_1 + 1.0079400000000001*H_1;
        ...
        while ( Tdiff/ Tnew > 1.0e-8){
            ...
            simple_interpolate_gamma(data, i);
            
            gammaH2_1 = data->gammaH2_1[threadID][i];
            dgammaH2_1_dT = 0.0; //data->dgammaH2_1_dT[threadID][i]; // iterations do not converge in some cases...
            ...
            dge_dT = -H2_1*T*dgammaH2_1_dT*kb/(density*mh*pow(gammaH2_1 - 1.0, 2)) + kb*(H2_1/(gammaH2_1 - 1.0) + H_1/(gamma - 1.0))/(density*mh);
        
            //This is the change in ge for each iteration
            dge = T*kb*(H2_1/(gammaH2_1 - 1.0) + H_1/(gamma - 1.0))/(density*mh) - ge;
...
int dengo_calculate_pressure_enzo( code_units* units, dengo_field_data *field_data){
    ...
    simple_data *data = simple_setup_data( FileLocation, NULL, NULL);
    ...
    for ( d = 0; d < ntimes; d++ ){
        ...
        simple_calculate_temperature(data,  input_batch , nstrip, nchem );
    ...
    if (nstrip_res > 0){
        ...
        simple_calculate_temperature(data,  input_batch , nstrip_res , nchem );
...
int dengo_estimate_cooling_time_enzo( code_units* units, dengo_field_data *field_data ){
    ...
    for ( d = 0; d < ntimes; d++ ){
        ...
        simple_calculate_cooling_timescale( cooling_time_batch, input_batch, nstrip, data);
    }

    if (nstrip_res > 0){
        input_batch        = &input[ntimes* nstrip * nchem];
        cooling_time_batch = &field_data->CoolingTime[ntimes*nstrip]; 
        simple_calculate_cooling_timescale( cooling_time_batch, input_batch, nstrip_res, data );    
    }
...
int simple_calculate_cooling_timescale( double *cooling_time, double *input, int nstrip, simple_data *data){
    ...
    flag = simple_calculate_temperature(data,  input , nstrip, nchem );
    if (flag > 0){
        // check if the temperature failed to converged
        return -1;    
    }
    simple_interpolate_rates(data, nstrip);
    double H2_1;
    double H_1;
    double ge;
    ...
    for ( i = 0; i < nstrip; i++ ){
        ...
        // Species: ge
        //
        dge_dt = 0.5*1.0/(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0)*(-h2formation_h2mcool[i]*H2_1*H_1 + h2formation_h2mheat[i]*pow(H_1, 3));
        dge_dt *= inv_mdensity;
...
int dengo_calculate_gamma( double* gamma_eff, simple_data *data, double *input, int nstrip  ){
    unsigned long int i, j, d, dims;
    int nchem     = 3;
    double H2_1;
    double H_1;
    ...
    for (i = 0; i < nstrip; i++ ){
        ...
        n_gamma_m1 = H2_1/(gammaH2_1 - 1.0) + H_1/(gamma - 1.0);
...
int simple_solve_chemistry( code_units *units, dengo_field_data *field_data, double dt ){
    ...
    */
    ...
    flag = dengo_evolve_simple(dtf, dt, z, input, rtol, atol, dims, data, temp);
...
int simple_solve_chemistry_enzo( code_units *units, dengo_field_data *field_data, double dt ){
    //-------------------------------------------------------------------------
    // Function: simple_solve_chemistry_enzo
    // Description: takes the same input as simple_solve_chemistry
...
int simple_solve_chemistry_dt( code_units *units, dengo_field_data *field_data, double* reltol, double* abstol, double dt ){
    // TODO:
    // this is not only called from the python modules 
    // but this is dumb...
    // this is almost the same as simple_solve_chemistry!!!
    ...
    */
    ...
    flag = dengo_evolve_simple(dtf, dt, z, input, reltol, abstol, dims, data, temp);
...
int calculate_equilibrium_abundance( simple_data *data, double *input, 
    ...
    // update the rates given the updated temperature from simple_data
    simple_interpolate_rates(data, nstrip);
             ...
             void *user_data, N_Vector tmp)
{
    ...
    int nstrip = 1;
    int nchem = 3;
    simple_data *data = (simple_data*)user_data; 
    ...
    int flag;
    flag = simple_calculate_temperature(data, y_arr, nstrip, nchem);
    if (flag > 0){
        return 1;    
    }
    ...
    // Species: H2_1
    //
    Ith(Jv, 1 ) = -k13[i]*H_1*v0 + Tge[i]*v2*(-H2_1*H_1*rk13[i] + pow(H_1, 3)*rk22[i]) + v1*(-k13[i]*H2_1 + 3*k22[i]*pow(H_1, 2));
    ...
    // Species: H_1
    //
    Ith(Jv, 2 ) = 2*k13[i]*H_1*v0 + Tge[i]*v2*(2*H2_1*H_1*rk13[i] - 2*pow(H_1, 3)*rk22[i]) + v1*(2*k13[i]*H2_1 - 6*k22[i]*pow(H_1, 2));
...
int calculate_rhs_simple(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    ...
    for ( i = 0; i < nstrip; i++ ){
        ...
        j = i * nchem;
        // Species: H2_1
        ydot_ptr[j] = -k13[i]*H2_1*H_1 + k22[i]*pow(H_1, 3);
        #ifdef SCALE_INPUT
        ...
        // Species: H_1
        ydot_ptr[j] = 2*k13[i]*H2_1*H_1 - 2*k22[i]*pow(H_1, 3);
        #ifdef SCALE_INPUT
        ydot_ptr[j] *= inv_scale[j];
        #endif
        ...
        // Species: ge
        ydot_ptr[j] = 0.5*1.0/(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0)*(-h2formation_h2mcool[i]*H2_1*H_1 + h2formation_h2mheat[i]*pow(H_1, 3));
        #ifdef SCALE_INPUT
        ydot_ptr[j] *= inv_scale[j];
        #endif
                                        ...
                                        N_Vector tmp3)
{
    ...
    */
    ...
    // simple_calculate_temperature(data, y_arr, nstrip, nchem);
    // simple_interpolate_rates(data, nstrip);
    ...
    for ( i = 0; i < nstrip; i++ ){
        ...
        SM_ELEMENT_D( J, j + 0, j + 0 ) = -k13[i]*H_1;
        ...
        SM_ELEMENT_D( J, j + 0, j + 1 ) = -k13[i]*H2_1 + 3*k22[i]*pow(H_1, 2);
        ...
        SM_ELEMENT_D( J, j + 0, j + 2 ) = -H2_1*H_1*rk13[i] + pow(H_1, 3)*rk22[i];
        ...
        SM_ELEMENT_D( J, j + 1, j + 0 ) = 2*k13[i]*H_1;
        ...
        SM_ELEMENT_D( J, j + 1, j + 1 ) = 2*k13[i]*H2_1 - 6*k22[i]*pow(H_1, 2);
        ...
        SM_ELEMENT_D( J, j + 1, j + 2 ) = 2*H2_1*H_1*rk13[i] - 2*pow(H_1, 3)*rk22[i];
        ...
        #endif
        SM_ELEMENT_D( J, j + 2, j + 0 ) *= inv_mdensity;
        
        // ge by H_1
        
        SM_ELEMENT_D( J, j + 2, j + 1 ) = 0.5*h2formation_ncrd1[i]*h2formation_ncrn[i]*pow(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0, -2.0)*(-h2formation_h2mcool[i]*H2_1*H_1 + h2formation_h2mheat[i]*pow(H_1, 3))/pow(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1, 2) + 0.5*(-h2formation_h2mcool[i]*H2_1 + 3*h2formation_h2mheat[i]*pow(H_1, 2))*1.0/(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0);
        ...
        #endif
        SM_ELEMENT_D( J, j + 2, j + 1 ) *= inv_mdensity;
        
        // ge by ge
        
        SM_ELEMENT_D( J, j + 2, j + 2 ) = 0.5*pow(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0, -2.0)*(-h2formation_h2mcool[i]*H2_1*H_1 + h2formation_h2mheat[i]*pow(H_1, 3))*(-1.0*h2formation_ncrn[i]*(-H2_1*rh2formation_ncrd2[i] - H_1*rh2formation_ncrd1[i])/pow(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1, 2) - 1.0*rh2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1)) + 0.5*1.0/(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0)*(-H2_1*H_1*rh2formation_h2mcool[i] + pow(H_1, 3)*rh2formation_h2mheat[i]);
        ...
        #endif
        SM_ELEMENT_D( J, j + 2, j + 2 ) *= inv_mdensity;
        SM_ELEMENT_D( J, j + 2, j + 2 ) *= Tge[i];
    }
    return 0;
}
                                        ...
                                        N_Vector tmp3)
{
    ...
    */
    ...
    for ( i = 0; i < nstrip; i++ ){
        ...
        matrix_data[ j + 0 ] = -k13[i]*H_1;

        
        // H2_1 by H_1
        colvals[j + 1] = i * nchem + 1 ;
        matrix_data[ j + 1 ] = -k13[i]*H2_1 + 3*k22[i]*pow(H_1, 2);

        
        // H2_1 by ge
        colvals[j + 2] = i * nchem + 2 ;
        matrix_data[ j + 2 ] = -H2_1*H_1*rk13[i] + pow(H_1, 3)*rk22[i];

        
        matrix_data[ j + 2] *= Tge[i];
        // H_1 by H2_1
        colvals[j + 3] = i * nchem + 0 ;
        matrix_data[ j + 3 ] = 2*k13[i]*H_1;

        
        // H_1 by H_1
        colvals[j + 4] = i * nchem + 1 ;
        matrix_data[ j + 4 ] = 2*k13[i]*H2_1 - 6*k22[i]*pow(H_1, 2);

        
        // H_1 by ge
        colvals[j + 5] = i * nchem + 2 ;
        matrix_data[ j + 5 ] = 2*H2_1*H_1*rk13[i] - 2*pow(H_1, 3)*rk22[i];

        
        matrix_data[ j + 5] *= Tge[i];
        // ge by H2_1
        colvals[j + 6] = i * nchem + 0 ;
        matrix_data[ j + 6 ] = -0.5*h2formation_h2mcool[i]*H_1*1.0/(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0) + 0.5*h2formation_ncrd2[i]*h2formation_ncrn[i]*pow(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0, -2.0)*(-h2formation_h2mcool[i]*H2_1*H_1 + h2formation_h2mheat[i]*pow(H_1, 3))/pow(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1, 2);

        
        matrix_data[j + 6] *= inv_mdensity;
        // ge by H_1
        colvals[j + 7] = i * nchem + 1 ;
        matrix_data[ j + 7 ] = 0.5*h2formation_ncrd1[i]*h2formation_ncrn[i]*pow(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0, -2.0)*(-h2formation_h2mcool[i]*H2_1*H_1 + h2formation_h2mheat[i]*pow(H_1, 3))/pow(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1, 2) + 0.5*(-h2formation_h2mcool[i]*H2_1 + 3*h2formation_h2mheat[i]*pow(H_1, 2))*1.0/(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0);

        
        matrix_data[j + 7] *= inv_mdensity;
        // ge by ge
        colvals[j + 8] = i * nchem + 2 ;
        matrix_data[ j + 8 ] = 0.5*pow(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0, -2.0)*(-h2formation_h2mcool[i]*H2_1*H_1 + h2formation_h2mheat[i]*pow(H_1, 3))*(-1.0*h2formation_ncrn[i]*(-H2_1*rh2formation_ncrd2[i] - H_1*rh2formation_ncrd1[i])/pow(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1, 2) - 1.0*rh2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1)) + 0.5*1.0/(h2formation_ncrn[i]/(h2formation_ncrd1[i]*H_1 + h2formation_ncrd2[i]*H2_1) + 1.0)*(-H2_1*H_1*rh2formation_h2mcool[i] + pow(H_1, 3)*rh2formation_h2mheat[i]);

        
        matrix_data[j + 8] *= inv_mdensity;
        matrix_data[ j + 8] *= Tge[i];
        rowptrs[ i * nchem +  0] = i * NSPARSE + 0;
        rowptrs[ i * nchem +  1] = i * NSPARSE + 3;
        rowptrs[ i * nchem +  2] = i * NSPARSE + 6;

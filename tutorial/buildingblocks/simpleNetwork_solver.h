...
typedef struct simpleNetwork_data {
    ...
    double scale[NTHREADS][4 * MAX_NCELLS ];
    double inv_scale[NTHREADS][4 * MAX_NCELLS];
...
void *setup_cvode_solver( rhs_f f, jac_f Jac,  int NEQ, 
        simpleNetwork_data *data, SUNLinearSolver LS, SUNMatrix A, N_Vector y, double reltol, N_Vector abstol);

int cvode_solver( void *cvode_mem, double *output, int NEQ, double *dt, simpleNetwork_data * data, N_Vector y, double reltol, N_Vector abstol );


simpleNetwork_data *simpleNetwork_setup_data( const char *FileLocation, int *NumberOfFields, char ***FieldNames);
void simpleNetwork_read_rate_tables(simpleNetwork_data*);
void simpleNetwork_read_cooling_tables(simpleNetwork_data*);
void simpleNetwork_read_gamma(simpleNetwork_data*);
void simpleNetwork_interpolate_gamma(simpleNetwork_data*, int );

void setting_up_extra_variables( simpleNetwork_data * data, double * input, int nstrip );

int dengo_evolve_simpleNetwork (double dtf, double &dt, double z,
...
int simpleNetwork_calculate_temperature(simpleNetwork_data *data, double *input, int nstrip, int nchem);
...
int dengo_calculate_gamma( double* gamma_eff, simpleNetwork_data*, double* input, int nstrip);
...
void ensure_species_conservation(double *input, double *mdensity, double *equil_array, simpleNetwork_data *data, int nstrip, unsigned long d, unsigned long dims, int nchem);